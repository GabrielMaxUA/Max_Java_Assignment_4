...
public class IO {
  private String fileWinName = "gameWinSave.txt";
  ...
  //creating save win.loss
  public void saveWinLoss(Warrior player, Warrior enemy){
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileWinName))) {
      writer.write(player.getWinCount() + " ");
      writer .write(player.getLooseCount() + "\n");

      writer.write(enemy.getWinCount() + " ");
      writer .write(enemy.getLooseCount() + " ");
  }
  catch (IOException e) {
    e.printStackTrace();
  }
...
public void loadWinLoss(Warrior player, Warrior enemy) {
  try (BufferedReader reader = new BufferedReader(new FileReader(fileWinName))) {
      String[] playerCount = reader.readLine().split(" ");
      player.setWinCount(Integer.parseInt(playerCount[0]));
      player.setLooseCount(Integer.parseInt(playerCount[1]));

      String[] enemyCount = reader.readLine().split(" ");
      enemy.setWinCount(Integer.parseInt(enemyCount[0]));
      enemy.setLooseCount(Integer.parseInt(enemyCount[1]));
  } catch (IOException e) {
      e.printStackTrace();
  }
}
